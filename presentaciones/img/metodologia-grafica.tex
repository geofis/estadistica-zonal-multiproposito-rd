...
\begin{tikzpicture}[
  ...
  \node (autenticar) [process, right of=instalar, xshift=4cm] {EE: \\ Crear proyecto \\ autenticar};
   \node (fuente) [io, right of=autenticar, xshift=4cm] {\textbf{Seleccionar/subir} \\ \textbf{fuente de datos en EE}};
   \node (representarfuente) [io, below of=fuente, yshift=-2cm] {\textbf{Representar fuente,} \\ \textbf{explorar valores}};


   \node (zonas) [io, left of=representarfuente, xshift=-4cm] {Seleccionar/subir \\ zonas \\ (eg.H3))};
  \node (end) [startstop, left of=zonas, xshift=-4cm] {\textbf{Repositorio de} \\ \textbf{Estadística zonal} \\ \textbf{multipropósito}};
